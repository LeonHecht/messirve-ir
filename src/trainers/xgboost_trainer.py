...
class XGBoostPairwiseTrainer:
    ...
    def train(self,
        ...
        if self.mode == 'classification':
            ...
            eval_sets   = [(self.X_train, self.y_train),
               (self.X_dev,   self.y_dev)]
            eval_groups = [self.group_train,
                        self.group_dev]
    ...
    def run(self, **train_kwargs):
        ...
        # self.train_weighted_ranking(**train_kwargs)
        self.evaluate()
...
if __name__ == "__main__":
    ...
    train_tsv = os.path.join(base, "datasets", "cross_encoder", "bce_6x_synthetic_train.tsv")
    dev_tsv   = os.path.join(base, "datasets", "cross_encoder", "bce_6x_synthetic_dev.tsv")
    test_tsv  = os.path.join(base, "datasets", "cross_encoder", "bce_6x_synthetic_test.tsv")

    queries_path = os.path.join(base, "corpus", "consultas_sinteticas_380_filtered.tsv")
    ...
    rank_trainer = XGBoostPairwiseTrainer(
        train_tsv, dev_tsv, test_tsv, mode='ranking', queries_path=queries_path
    )
    rank_trainer.run()
